...
* Tenés un ``requirements.txt`` en **la raiz** de tu repo, con las dependencias python de tu proyecto, donde figura django y cualquier otra cosa que haga falta instalar con pip para que funcione, y que se puede usar con un ``pip install -r requirements.txt``. El formato dentro
del requirements debería ser, por ejemplo, ``Django==1.8``, la versión de cada cosa puede verse
usando ``pip list``.
